...
from .prompt_styles import STYLE_OFF_LABEL, get_style_options, infer_mode_category
...
_MODEL_CACHE_TTL_SECONDS = 60 * 60 * 24 * 30
...
def _load_prefs() -> dict[str, Any]:
    ...
    try:
        ...
        capabilities = payload.get("model_capabilities", {})
...
def _get_model_capability(model_name: str) -> tuple[bool, str]:
    ...
    prefs = _load_prefs()
    cache = prefs.get("model_capabilities", {})
    entry = cache.get(key)
    ...
    if isinstance(entry, dict):
        ...
        source = normalize_string_input(entry.get("source", "cache"), default="cache")
        if updated_at > 0 and (now - updated_at) <= _MODEL_CACHE_TTL_SECONDS:
    ...
    prefs["model_capabilities"] = cache
    _save_prefs(prefs)
    ...
    def execute(
        ...
        try:
            ...
            if context_paths:
                ...
                if not supports_vision and capability_source == "runtime":
                    _add_notice(
                        (
            ...
            def _build_cli_args(current_context_paths: list[str], force_prompt_pipe: bool = False) -> tuple[list[str], bool, str]:
                ...
                if current_context_paths:
                    context_lines = [f"- {path}" for path in current_context_paths]
                    prompt_parts.append(
                        "Context files:\n"
                        + "\n".join(context_lines)
                        + "\nInspect these local files before responding if relevant to the request."
                    )

                cli_prompt = "\n\n".join(prompt_parts).strip()
                cli_prompt = self.require_prompt(cli_prompt, input_name="prompt")

                # GitHub's programmatic CLI contract supports piping prompts on stdin.
                # Use that path when argv transport becomes risky on Windows.
            ...
            if notices:
                notice_block = "\n".join(f"[Copilot Notice] {notice}" for notice in notices)
                result_text = f"{notice_block}\n\n{result_text}"

            logger.info("GZ_CopilotAgent completed with model '%s'.", selected_model)
            return (result_text,)
